...
import time
import robot_controller #DODANE
import numpy as np
...
def depth_callback(depth_message):
    ...
    with TimeIt('Publish'):
        ...
        #DODANE
        trajectory = list()
        pose = [x, y, z, ang, width, depth_center]
        trajectory.append(pose)
        manipulator.move(trajectory, False, a=0.1, v=0.5)


depth_sub = rospy.Subscriber('/camera/depth/image', Image, depth_callback, queue_size=1)
img_sub = rospy.Subscriber('/camera/rgb/image_color', Image, img_callback, queue_size=1)
